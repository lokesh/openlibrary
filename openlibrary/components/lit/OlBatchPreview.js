...
import { api, olid } from '../../plugins/openlibrary/js/editing-mode/api.js';
import './OlDialog.js';
import './OLButton.js';
import './OlIcon.js';
 ...
 *
 * `show({ action, items, params })` runs a dry run against
 ...
 *
 * @fires ol-batch-applied - detail: { batch_id, action, keys, dropFromTray }
...
export class OlBatchPreview extends LitElement {
    ...
    async apply() {
        ...
        try {
            ...
            this.dispatchEvent(new CustomEvent('ol-batch-applied', {
                bubbles: true,
                composed: true,
                detail: { batch_id: this._result.batch_id, action: r.action, status: this._result.status, keys, dropFromTray: !!r.dropFromTray, reload: !!r.reload },
            }));
        } catch (e) {
    ...
    /** Records the batch touches, as passed by the tray; empty for callers that only send keys. */
    get records() {
    ...
    cardLines(r) {
        ...
        const by = [...new Set(r.authors || [])].join(', ');
    ...
    renderChanges() {
        ...
        return html`
            <div class="changes">
                <table>
                    <thead><tr><th>Record</th><th>${this.t('field')}</th><th>${this.t('changes')}</th></tr></thead>
                    <tbody>
                        ...
                        ${changes.length > 400 ? html`<tr><td colspan="3" class="muted">… ${changes.length - 400} more</td></tr>` : nothing}
                    </tbody>
                </table>
            </div>`;
    }
        ...
        return html`
            <ol-dialog label=${r.title || p?.label || this.t('preview')} width="large" fullscreen-on-mobile @ol-after-close=${() => { this._open = false; }}>
                <div class="body" aria-busy=${this._busy ? 'true' : 'false'}>
                    ${this.renderRecords()}
                    ${p && !isChecks && p.mode === 'request' ? html`<span class="mode">A super-librarian will review this before it is applied.</span>` : nothing}
