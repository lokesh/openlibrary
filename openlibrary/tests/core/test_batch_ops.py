...
class FakeSite:
    ...
    def things(self, query, details=False):
        if query.get("type") == "/type/edition" and query.get("works"):
            return [k for k, d in self.docs.items() if any(w["key"] in query["works"] for w in d.get("works", []))]
        return []
...
def fakes(monkeypatch):
    ...
    def create(username, action, params, items, changes, warnings, overrides, status="previewed", comment=None, mrid=None):
        bid = len(rows) + 1
        rows[bid] = {
            "id": bid,
            "username": username,
...
def test_move_editions_to_existing_and_new_work(fakes):
    ...
    assert new["creates"] == ["/works/OL101W"]
