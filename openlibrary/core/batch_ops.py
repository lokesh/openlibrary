...
class MoveEditionsAction(Action):
    ...
    def plan(self, docs, params, ctx):
        ...
        if target_param == "new":
            ...
            key = site.get().new_key("/type/work")
            work = {
                "key": key,
                "type": {"key": "/type/work"},
...
class SetAuthorAction(Action):
    ...
    def plan(self, docs, params, ctx):
        ...
        if include_editions and works:
            ...
            ed_keys = site.get().things({"type": "/type/edition", "works": work_keys, "limit": 1000}) if work_keys else []
            ed_keys = [k for k in ed_keys if k not in docs]
            if ed_keys:
...
class DeleteAction(Action):
    ...
    def plan(self, docs, params, ctx):
        ...
        if params.get("include_editions"):
            ...
            if work_keys:
                keys += [k for k in site.get().things({"type": "/type/edition", "works": work_keys, "limit": 1000}) if k not in docs]
        for key in keys:
...
# The first release ships the loop librarians already run (tag, merge, list) plus
# the two edits that replace the classic drag-and-drop. The rest stay registered
# but the endpoint refuses them until they are enabled here; the tray hides them
# behind the same gate (V1_ACTIONS in components/lit/OlLibrarianTray.js).
ENABLED_ACTIONS: frozenset[str] = frozenset({"tag", "move_editions", "set_author"})
...
def run(
    ...
    ctx = Ctx(username=user.key.split("/")[-1], is_super=bool(user.is_super_librarian_or_higher()))
    ...
    can_apply = not blocks or (ctx.is_super and mode == "apply" and all(b["code"] in overrides for b in blocks))
...
def apply_requested(user: Any, batch_id: int, comment: str | None = None) -> dict[str, Any]:
    """A super-librarian applies a batch a librarian requested. The plan is rebuilt against current records."""
    ...
    overrides = list(batch.get("overrides") or [])
    blocks = [w for w in warnings if w["level"] == "block" and w["code"] not in overrides]
    if blocks:
        raise BatchError("The batch has blocking warnings; review it before applying.", 409, warnings=warnings)
    stored_items = _stored_items(docs, plan.creates)
    return _apply(
...
def revert(user: Any, batch_id: int, key: str | None = None) -> dict[str, Any]:
